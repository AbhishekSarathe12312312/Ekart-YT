...
export const verifyPayment = async (req, res) => {
  try {
    ...
    // Handle user-cancelled or failed checkout modal
    if (paymentFailed) {
      const order = await Order.findOneAndUpdate(
        { razorpayOrderId: razorpay_order_id, userId }, // Scope to user for safety
        { status: "Failed" },
        { new: true },
      );
      return res
...
export const getSalesData = async (req, res) => {
  try {
    ...
    const [totalUsers, totalProducts, totalOrders, totalSaleAgg, salesByDate] =
      await Promise.all([
        User.countDocuments({}),
        ...
        Order.aggregate([
          {
            $match: {
              status: "Paid",
              createdAt: { $exists: true, $ne: null, $gte: thirtyDaysAgo },
            },
          },
          {
            $group: {
